-- Add 'laundry' as a first-class business type (service-based: drop-off,
-- per-service billing, pay-later/on-collection).
-- Run after ADD_HARDWARE_WHOLESALE_ORDER_SETTINGS.sql.
...
ALTER TABLE public.supermarkets
  ADD CONSTRAINT supermarkets_business_type_check CHECK (business_type IN
    ('supermarket', 'pharmacy', 'hotel', 'boutique', 'restaurant_cafe', 'wholesale', 'hardware', 'factory', 'laundry'));
...
BEGIN
  ...
  IF COALESCE(p_business_type, '') NOT IN ('supermarket', 'pharmacy', 'hotel', 'boutique', 'restaurant_cafe', 'wholesale', 'hardware', 'factory', 'laundry') THEN
