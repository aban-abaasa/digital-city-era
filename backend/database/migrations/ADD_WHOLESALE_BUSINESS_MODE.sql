-- Add wholesale as a first-class Supermarketa business mode.
-- Run this on existing deployments after ADD_PHARMACY_FLEXIBLE_INVENTORY.sql.

ALTER TABLE public.supermarkets
  DROP CONSTRAINT IF EXISTS supermarkets_business_type_check;

ALTER TABLE public.supermarkets
  ADD CONSTRAINT supermarkets_business_type_check
  CHECK (business_type IN ('supermarket', 'pharmacy', 'hotel', 'boutique', 'restaurant_cafe', 'wholesale', 'hardware', 'factory'));
...
BEGIN
  ...
  IF COALESCE(p_business_type, '') NOT IN
    ('supermarket', 'pharmacy', 'hotel', 'boutique', 'restaurant_cafe', 'wholesale', 'hardware', 'factory') THEN
    RETURN jsonb_build_object('success', false, 'error', 'Invalid business type');
  END IF;

  SELECT id INTO v_existing_id FROM public.supermarkets
   WHERE owner_user_id = v_auth_id LIMIT 1;
  IF v_existing_id IS NOT NULL THEN
    RETURN jsonb_build_object('success', true, 'supermarket_id', v_existing_id, 'already_existed', true);
  END IF;

  v_token := gen_random_uuid();
  SELECT id INTO v_pichin_business_id FROM public.business_profiles
   WHERE user_id = v_auth_id ORDER BY created_at ASC LIMIT 1;

  INSERT INTO public.supermarkets
    (name, description, phone, email, address, city, country, owner_user_id,
     onboarding_token, is_active, business_type, pichin_business_profile_id, created_at, updated_at)
  VALUES
    (trim(p_name), p_description, p_phone, p_email, p_address, p_city, p_country,
     v_auth_id, v_token, true, COALESCE(p_business_type, 'supermarket'),
     v_pichin_business_id, now(), now())
  RETURNING id INTO v_new_id;

  SELECT id INTO v_internal_user_id FROM public.users
   WHERE auth_id = v_auth_id OR id = v_auth_id LIMIT 1;
  IF v_internal_user_id IS NOT NULL THEN
    UPDATE public.users SET role = 'admin', supermarket_id = v_new_id, updated_at = now()
  ...
  RETURN jsonb_build_object('success', true, 'supermarket_id', v_new_id,
    'onboarding_token', v_token, 'business_type', COALESCE(p_business_type, 'supermarket'),
