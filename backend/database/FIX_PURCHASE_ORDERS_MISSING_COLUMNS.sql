-- ============================================================
-- FIX: purchase_orders is missing several columns on this
-- environment (amount_paid_ugx, delivery_address, ... surfaced one
-- at a time as PGRST204 "column not found" errors while testing
-- order creation/payment tracking).
--
-- This DB never ran migrations/ADD_PURCHASE_ORDERS_COLUMNS.sql.
-- Rather than patch columns one at a time as each error surfaces,
-- this runs that full migration (safe — every column addition is
-- guarded with IF NOT EXISTS, so anything already present is a
-- no-op) plus a schema-cache reload so Supabase picks it up
-- immediately instead of erroring for a few more minutes.
--
-- Safe to re-run.
-- ============================================================
...
-- Force PostgREST to reload its schema cache immediately, otherwise the
-- 'column not found' error can persist for a bit even after the ALTER above.
NOTIFY pgrst, 'reload schema';

SELECT 'purchase_orders schema caught up — all columns from ADD_PURCHASE_ORDERS_COLUMNS.sql present.' AS status;
