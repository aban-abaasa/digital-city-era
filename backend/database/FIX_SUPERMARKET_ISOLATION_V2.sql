-- ============================================================================
-- FIX: SUPERMARKET ISOLATION V2 - EACH ADMIN GETS THEIR OWN SUPERMARKET
-- ============================================================================
...
-- Check and add supermarket_id to suppliers if missing
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.suppliers ADD COLUMN supermarket_id UUID REFERENCES public.supermarkets(id) ON DELETE CASCADE;
    CREATE INDEX IF NOT EXISTS idx_suppliers_supermarket ON public.suppliers(supermarket_id);
    RAISE NOTICE '✅ Added supermarket_id column to suppliers table';
  ELSE
    RAISE NOTICE '✓ supermarket_id column already exists in suppliers table';
  END IF;
END $$;
...
-- Users can ONLY read suppliers from their own supermarket OR if supplier is not assigned yet
CREATE POLICY "Users can read own supermarket suppliers" ON public.suppliers
  FOR SELECT
  USING (
    supermarket_id IS NULL  -- Suppliers not yet assigned to a supermarket
    OR
    EXISTS (
      SELECT 1 FROM public.users
      WHERE users.auth_id = auth.uid()
...
CREATE POLICY "Admins can update own supermarket suppliers" ON public.suppliers
  FOR UPDATE
  USING (
    supermarket_id IS NULL  -- Can assign to their supermarket
    OR
    EXISTS (
      SELECT 1 FROM public.users
      WHERE users.auth_id = auth.uid()
...
-- View to see supermarket ownership
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '╔════════════════════════════════════════════════════════════════╗';
  RAISE NOTICE '║  ✅ SUPERMARKET ISOLATION FIX V2 COMPLETE!                     ║';
  RAISE NOTICE '╚════════════════════════════════════════════════════════════════╝';
  RAISE NOTICE '';
  RAISE NOTICE '📋 WHAT WAS FIXED:';
  RAISE NOTICE '  ✅ Added supermarket_id to suppliers table (if missing)';
  ...
  RAISE NOTICE '  3. Run: SELECT * FROM public.supermarket_ownership;';
  RAISE NOTICE '';
END $$;
