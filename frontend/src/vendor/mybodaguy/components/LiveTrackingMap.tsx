 ...
 * ride or delivery. Shows the pickup and drop-off pins plus the rider's
 * real position, pushed the instant it changes via a realtime subscription
 * on mbg_riders.current_lat/current_lng — same postgres_changes pattern
 * already used for chat and for pushing new ride requests to a rider
 * instantly.
 *
 * Ported from mybodaguy's src/mybodaguy/components/LiveTrackingMap.tsx —
 * only the Location import path changed (this app's data/locationTypes
 * instead of mybodaguy's data/mockLocations); everything else, including
 * the shared mbg_riders realtime channel, is unchanged since both apps
 * point at the same Supabase project (see services/supabaseClient.ts).
 */
