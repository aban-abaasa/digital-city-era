// Ported from mybodaguy's src/mybodaguy/components/JourneyRideOptions.tsx —
// same logic, plain Tailwind (no mybodaguy "classic" stylesheet here).
...
export default function JourneyRideOptions({
  ...
  const chip = (active: boolean) =>
    `flex items-center justify-center gap-1.5 py-2 rounded-lg text-[13px] font-semibold border-2 transition-all ${
      active ? 'border-orange-500 bg-orange-50 text-orange-700' : 'border-slate-200 text-slate-500'
    }`;

  return (
    <details className="border border-slate-200 rounded-lg p-3" open={value.powerType !== 'any' || value.umbrella || !!value.companyId}>
      ...
      <div className="mt-3 space-y-3">
        {bike && (
          <div>
            <span className="block text-xs font-medium text-slate-500 mb-1.5">Bike type</span>
        ...
        {bike && (
          <label className="flex cursor-pointer items-center gap-2.5 text-sm text-slate-700">
            <input type="checkbox" checked={value.umbrella} onChange={(e) => onChange({ ...value, umbrella: e.target.checked })} className="h-4 w-4 accent-orange-500" />
            <Umbrella size={15} className="text-orange-500" /> Rider brings an umbrella
          </label>
        )}
        {companies.length > 0 && (
          <div>
            <label className="flex items-center gap-1.5 text-xs font-medium text-slate-500 mb-1.5" htmlFor="jb-ride-company"><Building2 size={13} /> Transport company</label>
            <select id="jb-ride-company" className="w-full border rounded-lg p-3 bg-white text-slate-900" value={value.companyId} onChange={(e) => onChange({ ...value, companyId: e.target.value })}>
