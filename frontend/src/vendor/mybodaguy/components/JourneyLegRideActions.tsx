// Ported from mybodaguy's src/mybodaguy/components/JourneyLegRideActions.tsx —
// same logic, plain Tailwind.
...
export default function JourneyLegRideActions({ leg, customerId }: { leg: JourneyLeg; customerId: string }) {
  ...
  if (awaitingDispatch) {
    ...
    const when = due ? due.toLocaleString([], { weekday: 'short', hour: '2-digit', minute: '2-digit' }) : '';
    return (
      <div className="flex items-start gap-2 rounded-lg border border-orange-200 bg-orange-50 p-3 text-xs text-slate-600">
        {inFuture ? <Clock size={14} className="mt-0.5 shrink-0 text-orange-500" /> : <Search size={14} className="mt-0.5 shrink-0 text-orange-500" />}
        <span>
          {inFuture
            ? leg.leg_type === 'local_pickup'
              ? `Your driver will be assigned around ${when} — timed so you reach the airport in good time.`
              : `Your driver will be assigned around ${when}, after your flight lands.`
  ...
  if (ride.status === 'pending') {
    return (
      <div className="flex items-start gap-2 rounded-lg border border-orange-200 bg-orange-50 p-3 text-xs text-slate-600">
        <Loader2 size={14} className="mt-0.5 shrink-0 animate-spin text-orange-500" />
  ...
  return (
    <>
      <button
        ...
        onClick={open}
        disabled={opening}
        className="inline-flex min-h-[40px] w-full items-center justify-center gap-2 rounded-lg border border-orange-300 bg-white px-3 text-sm font-semibold text-orange-700 hover:bg-orange-50 disabled:opacity-60"
      >
