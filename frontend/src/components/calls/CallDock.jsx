 ...
 * CallDock.jsx.
 */
const CallDock = ({ call, dark = false }) => {
  ...
  const gradient = 'from-cyan-500 to-violet-600';
  ...
  return (
    <div className={`border-b px-3 py-2.5 ${dark ? 'border-white/10 bg-[#0b1220]' : 'border-slate-200 bg-slate-100'}`}>
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className={`truncate text-sm font-medium ${dark ? 'text-slate-100' : 'text-slate-800'}`}>Calling {call.peerName || 'them'}…</p>
      ...
      {call.callState === 'ringing-in' && (
        <div className="flex items-center gap-3">
          <span className={`flex h-9 w-9 flex-shrink-0 animate-bounce items-center justify-center rounded-full bg-gradient-to-br ${gradient} text-white`}>
            {call.isVideo ? <FiVideo className="h-4 w-4" /> : <FiPhone className="h-4 w-4" />}
          </span>
          <div className="min-w-0 flex-1">
            <p className={`truncate text-sm font-medium ${dark ? 'text-slate-100' : 'text-slate-800'}`}>{call.peerName || 'Someone'} is calling</p>
            <p className={`text-[11px] ${dark ? 'text-slate-500' : 'text-slate-400'}`}>{call.isVideo ? 'Video call' : 'Audio call'}</p>
          </div>
          <button
      ...
      {call.callState === 'active' && (
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {call.micOn ? <FiMic className="h-3.5 w-3.5" /> : <FiMicOff className="h-3.5 w-3.5" />}
            </button>
            {call.isVideo && (
              <button
              ...
              >
                {call.camOn ? <FiVideo className="h-3.5 w-3.5" /> : <FiVideoOff className="h-3.5 w-3.5" />}
              </button>
            )}
            <button
              onClick={call.endCall}
