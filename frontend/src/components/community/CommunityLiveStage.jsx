...
// Chat drawer over the live video — reuses whichever board the audience
// already reads/writes (landing_messages for Community, the team's own
// conversation for "My Store") rather than inventing separate live-stream
// storage. `onLike` is optional — Team messages have no like feature, so
// the reaction button only renders when a handler is passed (Community).
    ...
    <div className={`absolute inset-x-0 bottom-0 z-10 flex ${isMobile ? 'max-h-[38%]' : 'max-h-[55%]'} flex-col rounded-t-2xl bg-black/70 backdrop-blur-md`}>
          ...
          <div key={m.id} className="rounded-xl bg-white/10 px-2.5 py-1.5 text-sm text-white">
            ...
            {onLike && (
              <button
                ...
                className={`mt-0.5 inline-flex items-center gap-1 rounded-full px-1.5 py-0.5 text-[10px] font-medium ${m.likedByMe ? 'text-cyan-300' : 'text-white/60 hover:text-white'}`}
              >
                <FiThumbsUp className="h-3 w-3" /> {m.likeCount || 0}
              </button>
            )}
          </div>
        ))}
      ...
      <div className="flex items-center gap-2 px-4 pt-1" style={{ paddingBottom: 'max(0.75rem, env(safe-area-inset-bottom))' }}>
        ...
        <button
          ...
          className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-cyan-500 to-violet-600 text-white shadow-lg transition disabled:opacity-40"
          title="Send"
        >
 ...
 * `live.role` is 'broadcasting' or 'watching'. Reused for both the public
 * Community "Go Live" feature and the "My Store" staff-only group video
 * call, same one-tap takeover, just labeled for whichever audience `live`
 * (a useCommunityLive instance) is scoped to. Ported from ICAN's
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex flex-col bg-black">
      <div className="relative flex-1 bg-slate-900">
        ...
        <div className="absolute inset-x-0 top-0 flex items-center justify-between bg-gradient-to-b from-black/70 via-black/30 to-transparent px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <span className="flex items-center gap-1 rounded-full bg-black/40 px-2.5 py-1 text-xs font-medium text-white/90 backdrop-blur-sm">
              <FiEye className="h-3 w-3" /> {live.viewerCount}
            </span>
        ...
        {!chatOpen && (
          <button
            ...
            title="Show chat"
          >
            <FiMessageCircle className="h-4 w-4" /> Chat {messages.length > 0 && `(${messages.length})`}
          </button>
        )}
      ...
      >
        {isBroadcaster && (
          <ToolbarButton
            icon={live.micOn ? <FiMic className="h-5 w-5" /> : <FiMicOff className="h-5 w-5" />}
        ...
        {isBroadcaster && (
          <ToolbarButton
            icon={live.camOn ? <FiVideo className="h-5 w-5" /> : <FiVideoOff className="h-5 w-5" />}
